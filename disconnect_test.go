...
func TestReconnect(t *testing.T) {
	...
	if err = srvWs.Stop(); err != nil {
		t.Fatal(err)
	} else if err = srvWs.KillConnections(); err != nil {
		t.Fatal(err)
	}

	// wait for ws disconnect & reconnect period, assert FIX logoff msgs
	fix, err = fixOrd.WaitForMessageWithWait(OrderSessionID, 2, time.Second*20)
	if err != nil {
		t.Fatal(err)
	}
	err = checkFixTags(fix, "35=5", "49=BFXFIX", "56=EXORG_ORD")
	if err != nil {
		t.Fatal(err)
	}

	// start ws again
	if err = srvWs.Start(); err != nil {
		t.Fatal(err)
	}

	// wait for ws to reconnect, get a sequence number reset
	fix, err = fixOrd.WaitForMessageWithWait(OrderSessionID, 3, time.Second*20)
	if err != nil {
		t.Fatal(err)
	}
	err = checkFixTags(fix, "35=4", "49=BFXFIX", "56=EXORG_ORD")
	if err != nil {
		t.Fatal(err)
	}
