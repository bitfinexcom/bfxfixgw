...
func (w *Websocket) FIX42OrderSnapshotHandler(os bitfinex.OrderSnapshot, sID quickfix.SessionID) {
	...
	for _, o := range os {
		quickfix.SendToTarget(convert.FIX42ExecutionReportFromOrder(bitfinex.Order(o), p.BfxUserID(), enum.ExecType_ORDER_STATUS), sID)
	}
	return
}
...
func (w *Websocket) FIX42OrderCancelHandler(o bitfinex.OrderCancel, sID quickfix.SessionID) {
	...
	quickfix.SendToTarget(convert.FIX42ExecutionReportFromOrder(bitfinex.Order(o), p.BfxUserID(), enum.ExecType_CANCELED), sID)
	return
}
